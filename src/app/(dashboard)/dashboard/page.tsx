...
export default async function DashboardPage() {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="font-mono text-[10px] text-text-muted tracking-widest pl-4">
            {teams.length} UNIT{teams.length !== 1 ? "S" : ""} REGISTERED
          </p>
        </div>
        <Link
      ...
      <div className="mt-12 border-t border-border pt-4 flex items-center justify-between">
    ...
    <Link href={`/dashboard/teams/${team.id}`} className="block group">
      <div className="mtc-panel bg-bg-surface p-5 transition-all duration-150 hover:bg-bg-elevated hover:border-accent/40 cursor-pointer">
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-border">
          ...
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-text-muted group-hover:text-accent transition-colors">
